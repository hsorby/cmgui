...
/**
 * @brief process_multiple_options
 * @param state
 * @param modifier_table
 * @return
 */
...
/**
 * Destroy an option table.
 *
 * @param option_table_address
 * @return
 */
	...
	void *displayTypenameVoid)
{
	...
	if (state && enumValueAddress && displayTypename)
	{
		...
		if (currentToken)
		{
			...
			if (!Parse_state_help_mode(state))
			{
				do
				{
					...
					{
                        print_message(ERROR_MESSAGE, "Invalid %s %s", displayTypename, currentToken);
			...
			else
			{
				...
				while ((enumValueString = enumToString::toString(static_cast<enumType>(value))) != nullptr)
				{
					if (first)
					{
                        print_message(INFORMATION_MESSAGE, "%s", enumValueString);
						first = false;
					}
					else
                        print_message(INFORMATION_MESSAGE, "|%s", enumValueString);
		...
		{
            print_message(ERROR_MESSAGE, "Missing %s", displayTypename);
			display_parse_state_location(state);
			return_code = 0;
		}
	}
	...
	{
        print_message(ERROR_MESSAGE, "set %s.  Invalid argument(s)", displayTypename);
		return_code = 0;
	}
	return (return_code);
}
